...
public class LogParser_java_05 implements IPQuery, UserQuery, DateQuery, EventQuery, QLQuery {
    private Path logDir;
    private List<Logs> logs = new ArrayList<>();
    private DateFormat simpleDateFormat = new SimpleDateFormat("d.M.yyyy H:m:s");

    public LogParser_java_05(Path logDir) {
        this.logDir = logDir;
        readLogs();
    }
    ...
    private Object getCurrentValue(Logs logs, String field) {
        Object value = null;
        switch (field) {
            case "ip": {
                Command method = new GetIpCommand(logs);
                value = method.execute();
                break;
            }
            case "user": {
                Command method = new GetUserCommand(logs);
                value = method.execute();
                break;
            }
            case "date": {
                Command method = new GetDateCommand(logs);
                value = method.execute();
                break;
            }
            case "event": {
                Command method = new GetEventCommand(logs);
                value = method.execute();
                break;
            }
            case "status": {
                Command method = new GetStatusCommand(logs);
                value = method.execute();
                break;
            }
        }
    ...
    private class GetUserCommand extends Command {
        public GetUserCommand(Logs logEntity) {
            this.logs = logEntity;
        }
    ...
    private class GetDateCommand extends Command {
        public GetDateCommand(Logs logs) {
            this.logs = logs;
        }
    ...
    private class GetEventCommand extends Command {
        public GetEventCommand(Logs logs) {
            this.logs = logs;
        }

        @Override
        Object execute() {
            return logs.getEvent();
        }
    }

    private class GetStatusCommand extends Command {
        public GetStatusCommand(Logs logs) {
            this.logs = logs;
        }

        @Override
        Object execute() {
            return logs.getStatus();
        }
    }
    ...
    */
    ...
    public Set<Object> execute(String query) {
        ...
        for (int i = 0; i < logs.size(); i++) {
            result.add(getCurrentValue(logs.get(i), field));
        }
        return result;
    }
}
