...
public class LogParser_04 implements IPQuery, UserQuery, DateQuery, EventQuery {
    private Path logDir;
    private List<Logs> logs = new ArrayList<>();
    private DateFormat simpleDateFormat = new SimpleDateFormat("d.M.yyyy H:m:s");

    public LogParser_04(Path logDir) {
        this.logDir = logDir;
        readLogs();
    }
...
//                        int taskAttempt = result.get(task) + 1;
//                        result.put(task, taskAttempt);
//                    }
//                    else result.put(task, 1);
